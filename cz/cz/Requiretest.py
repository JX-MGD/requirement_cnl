from cz.cz import ToJson
from cz.cz import Postprocessing
from cz.cz import my_functions
...
def handleRequire(text):
    ...
    pos = model(text, target="POS")
    # 结果用两个列表封装
    nodeDataArray_temporary = []
    linkDataArray_temporary = []
    for answerlist, poslist in zip(answer, pos):
        # 给定依存关系列表
        dependency_list = answerlist
        pos_list = poslist
